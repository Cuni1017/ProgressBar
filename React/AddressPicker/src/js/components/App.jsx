...
class App extends Component {
  ...
  checkReceiptTypeIsReady = () => {
    ...
    if (this.state.receipt.receiptType == 2) {
      ...
      this.state.receipt.receiptType == 3 && //選擇公司，且有填寫統一編號
      this.state.receipt.taxId != ""
    ) {
      result = true;
    }
    return result;
  ...
  checkAddressPickerIsReady = () => {
    let result = true;
    let fullAddressArray = Object.keys(this.state.fullAddress);
    fullAddressArray.forEach((element) => {
      //   console.log(this.state.fullAddress[element]);
      if (this.state.fullAddress[element] == "") result = false;
    });
    return result;
  };
  ...
  render = () => {
    return (
      <form>
        ...
        <Grid>
          <Row>
            {/* 傳state進ReceiptType當props 避免重複render */}
